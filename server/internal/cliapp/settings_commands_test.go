...
func TestReadSettingsPayloadValidatesPartialPatch(t *testing.T) {
	...
	if _, err := readSettingsPayload(osFileSystem{}, `{"Unknown":1}`, ""); err == nil {
		t.Fatal("unknown JSON field unexpectedly accepted")
	}

	if _, err := readSettingsPayload(osFileSystem{}, `{"EnableDebug":true}`, ""); err == nil || !strings.Contains(err.Error(), "config.yml") {
		t.Fatalf("EnableDebug JSON error = %v", err)
	}

	if _, err := readSettingsPayload(osFileSystem{}, `{}`, ""); err == nil {
		t.Fatal("empty settings patch unexpectedly accepted")
	}

	if _, err := readSettingsPayload(osFileSystem{}, `{"ConnectionsLimit":50} {}`, ""); err == nil {
